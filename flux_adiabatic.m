function [E, F] = flux_adiabatic(FD_method, U, dx, dy, mu, k, R, cv) 
    [~, nx, ny] = size(U);
    ...
    % Enforce adiabatic wall condition
    F(4,2:end,1) = (Et(2:end,1) + p(2:end,1)) .* v(2:end,1) - v(2:end,1) .* tau_yy(2:end,1) - u(2:end,1) .* tau_xyF(2:end,1);
end