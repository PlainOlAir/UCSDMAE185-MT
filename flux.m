function [E, F] = flux(FD_method, U, dx, dy, mu, k, R, cv) 
    ...
    if strcmpi(FD_method,'forward')
        ...
        tau_xy = mu.*(ddx_fwd(u,dy,2) + ddx_fwd(v,dx,1));
        tau_xx = 2*(mu.*ddx_fwd(u,dx,1) - (1/3)*div_u.*u);
        tau_yy = 2*(mu.*ddx_fwd(v,dy,2) - (1/3)*div_u.*v);
        qx = -k.*ddx_fwd(T,dx,1);
        qy = -k.*ddx_fwd(T,dy,2);
    elseif strcmpi(FD_method,'backward')
        div_u = ddx_bwd(u,dx,1) + ddx_bwd(v,dy,2);
        tau_xy = mu.*(ddx_bwd(u,dy,2) + ddx_bwd(v,dx,1));
        tau_xx = 2*(mu.*ddx_bwd(u,dx,1) - (1/3).*div_u.*u);
        tau_yy = 2*(mu.*ddx_bwd(v,dy,2) - (1/3).*div_u.*v);
        qx = -k.*ddx_bwd(T,dx,1);
        qy = -k.*ddx_bwd(T,dy,2);
    else
